 ...
 * @return
 */
uint8_t R_FlashEraseRange2(uint32_t start_addr, uint32_t bytes);
char eraseFlashAddr(uint32_t flash_addr) {
	char bool1 = false;
	for (int i = flash_addr; i < flash_addr + 2048; i += 64) {
		wait_FRDY(10);
		if (FLASH.FASTAT.BYTE != 0x10) {
			FLASH.FASTAT.BYTE = 0x10;
			cmt_wait(1);
		}
		uint8_t ret = R_FlashEraseRange(i, 32);
 ...
 * @return
 */
char eraseFlashBrock(uint8_t brock) {
//	FLASH.FRESETR.BIT.FRESET = 1;
	if (FLASH.FASTAT.BYTE != 0x10) {
		FLASH.FASTAT.BYTE = 0x10;
//		FLASH.FRESETR.BIT.FRESET = 1;
	}
	eraseFlashAddr(g_flash_BlockAddresses[brock]);
	uint8_t ret = R_FlashErase(brock);
	return ret;
}
...
void readMaze(int db) {
	...
	trans_farm_ = false;
	uint32_t address = g_flash_BlockAddresses[db];
 ...
 */
//-----------------------------------------------------------------//
char erase_check(uint32_t org, uint32_t len) {
	if (mode_ != PE) {
		turn_pe_();
	}
	...
	*(FCU_BYTE_PTR) FACI_CMD_AREA = 0xD0;
	...
	if (cnt == 0) {  // time out
		turn_break_();
		error_ = TIMEOUT;
		myprintf("FACI 'erase_check' timeout\r\n");
		return false;
	}
	if (FLASH.FASTAT.BIT.CMDLK == 0) {
		...
		error_ = LOCK;
		myprintf("FACI 'erase_check' lock fail\r\n");
		return false;
	}
}
...
char erase(uint32_t org) {
	...
	*(FCU_BYTE_PTR) FACI_CMD_AREA = 0x20;
	*(FCU_BYTE_PTR) FACI_CMD_AREA = 0xD0;
...
char erase_all() {
	for (char i = 0; i < 16; i++) {
		char res = erase(addrList[i]);
		if (!res) {
			myprintf("erase error \r\n");
			return false;
		}
	}
	return true;
 ...
 */
//-----------------------------------------------------------------//
char write2(uint32_t addr, const void* data, uint32_t len) {
//	if (org >= data_flash_size) {
//		error_ = ADDRESS;
//		return false;
//	}

	if ((addr + len) > data_flash_size) {
		len = data_flash_size - addr;
	}
...
void testFcu() {
	...
	while (1) {
		...
		myprintf("%d \r\n", mode);
		if (mode == 0) {
			char res1 = erase(g_flash_BlockAddresses[BLOCK_DB0]);
			char res2 = erase(g_flash_BlockAddresses[BLOCK_DB1]);
			myprintf("delete result is %d ,%d \r\n", res1, res2);
			res = isFormatting(g_flash_BlockAddresses[BLOCK_DB0],
					g_flash_BlockAddresses[BLOCK_DB2] - 1);
			myprintf("format is %d \r\n", res);
			res = writeMap(g_flash_BlockAddresses[BLOCK_DB0]);
			myprintf("res=%d \r\n", res);
		} else if (mode == 1) {
			readMaze(BLOCK_DB0);
		} else if (mode == 2) {
			char res1 = erase(g_flash_BlockAddresses[BLOCK_DB2]);
			char res2 = erase(g_flash_BlockAddresses[BLOCK_DB3]);
			myprintf("delete result is %d ,%d \r\n", res1, res2);
