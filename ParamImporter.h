...
float getDataFlashData(long addr, int key) {
	...
	trans_farm_ = false;
	uint32_t address = g_flash_BlockAddresses[addr];
...
//	}
	...
	long target = g_flash_BlockAddresses[targetAddr];
	char res1 = erase(target);
	cmt_wait(100);
	myprintf("sizeof = %d\r\n", res1, sizeof(array));
	char res = R_FlashWrite(target, (uint32_t) array, sizeof(array));
